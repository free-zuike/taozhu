/** 系统设置（仅老板）：AI 配置（API 地址 / Key / 模型），供 AI 记账识别使用 */
import { Hono } from 'hono';
import { authMiddleware, adminOnly } from '../middleware/auth';
import { DEFAULT_AI_CONFIG, DEFAULT_TEXT_MODEL, DEFAULT_AUDIO_MODEL, type AiConfig } from '../services/ai-parse';
...
const KEY_TEXT_MODEL = 'ai_text_model';
const KEY_AUDIO_MODEL = 'ai_audio_model';
...
export async function getAiConfig(db: D1Database): Promise<AiConfig> {
  const [apiKey, baseUrl, model, textModel, audioModel] = await Promise.all([
    ...
    getSetting(db, KEY_TEXT_MODEL),
    getSetting(db, KEY_AUDIO_MODEL),
  ]);
  return {
    apiKey: apiKey?.trim() ?? '',
    baseUrl: baseUrl?.trim() || DEFAULT_AI_CONFIG.baseUrl,
    model: model?.trim() || DEFAULT_AI_CONFIG.model,
    textModel: textModel?.trim() || DEFAULT_TEXT_MODEL,
    audioModel: audioModel?.trim() || DEFAULT_AUDIO_MODEL,
  };
}
...
settingsRouter.get('/ai', async (c) => {
  ...
  return c.json({ has_key: Boolean(cfg.apiKey), base_url: cfg.baseUrl, model: cfg.model, text_model: cfg.textModel, audio_model: cfg.audioModel });
});

// PUT /api/v1/settings/ai — 保存（body: { api_key?, base_url?, model?, text_model?, audio_model? }；空字符串=清除该项回默认）
settingsRouter.put('/ai', async (c) => {
  const body = await c.req.json().catch(() => null) as {
    api_key?: string; base_url?: string; model?: string; text_model?: string; audio_model?: string;
  } | null;
  ...
  if (body?.text_model !== undefined) await upsert(KEY_TEXT_MODEL, body.text_model.trim());
  if (body?.audio_model !== undefined) await upsert(KEY_AUDIO_MODEL, body.audio_model.trim());
  const cfg = await getAiConfig(c.env.DB);
  return c.json({ has_key: Boolean(cfg.apiKey), base_url: cfg.baseUrl, model: cfg.model, text_model: cfg.textModel, audio_model: cfg.audioModel });
});