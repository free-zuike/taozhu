...
class _SalePageState extends State<SalePage> {
  ...
  double _autoCount(_Row row) {
    final opt = row.itemId == null ? null : _items.where((x) => x.id == row.itemId).firstOrNull;
    final price = opt?.prices.where((p) => p['id'] == row.priceId).firstOrNull;
    ...
    return Scaffold(
      ...
      body: ListView(
        ...
        children: [
          ...
          OutlinedButton.icon(
            onPressed: () => setState(
                () => _rows.add(_newRow()..happenedAt = _dateCtrl.text.trim())),
            ...
            children: [
              ...
              if (row.rowId.isNotEmpty) ...[
                ...
                IconButton(
                  ...
                  onPressed: () => showAttachmentViewer(context, 'sale_item', row.rowId, '出货明细行附件'),
                ),
              ],
              const SizedBox(width: 4),
              IconButton(
