...
/// 存储清理：列出更新安装包/临时文件等垃圾文件（Android 走系统 DownloadManager，
/// 桌面扫下载目录），勾选后删除。后续可在这里扩展"无效附件"等清理类型。
...
class _CacheFile {
  ...
  int size; // 字节；0=未知
  bool selected = false;
  _CacheFile(this.name, this.size);
}
    ...
    try {
      if (!kIsWeb && defaultTargetPlatform == TargetPlatform.android) {
        ...
        for (final item in list.cast<Map>()) {
          files.add(_CacheFile('${item['name'] ?? ''}', ((item['size'] as num?) ?? 0).toInt()));
        }
      } else if (!kIsWeb) {
        // 桌面：下载目录 + 临时目录
        Future<void> scan(Directory? dir) async {
          ...
          await for (final f in dir.list(followLinks: false)) {
            ...
            if (name.startsWith('taozhu-')) {
              files.add(_CacheFile(name, await f.length()));
            }
          }
        }
                              ...
                              children: [
                                for (final f in _files)
                                  CheckboxListTile(
                                    ...
                                    subtitle: Text(_fmtSize(f.size), style: TextStyle(fontSize: 12, color: c.textSub)),
