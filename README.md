...
- **记录**：出货记单（选店铺 → 商品 → 数量 → 自动带价，可改日期补录）、进货记单（同），AI 拍照识别小票自动填单
- **账本**：出货 / 进货 / 收款历史，按店铺+日期筛选，单笔可**编辑、删除**（纠错入口）
- **对账单**：按店铺 + 周期（本月/上月/自定义）汇总出货/收款/期末欠款明细，一键复制文本发送给客户
- **结账**：收款登记（金额/日期/方式/备注），欠款 = 累计出货 − 累计收款（按店铺）
- **统计**：工作台卡片、按店（出货/收款/欠款/毛利）、按月/按日、商品出货排行、自定义区间汇总
- **分类**：商品分类 / 店铺分类，两级（一级 → 子级，一级可挂多个子分类）
- **多用户**：老板/店员两角色（店员记单查看，管理操作仅老板；账号管理页面管理）
