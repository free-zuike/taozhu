...
通用进销存记账系统——管理客户、商品（多单位/进价/售价）、出货、进货、收款结账、两级分类与多用户权限。菜铺、批发、小店都能用，部署在 Cloudflare Workers（零主机）。

## 功能

- **商品管理**：商品 + 单位价格组合（同商品可 斤/袋/箱 多价：进价 → 售价），支持编辑、改价、停用价格、按名称搜索
...
- **对账单**：按店铺 + 周期（本月/上月/自定义）汇总出货/收款/期末欠款明细，一键复制文本发送给客户；**Excel 导出支持排版模板**（内置标准/旬段汇总，可自定义模板并预览），**按模板打印**（逐单明细/每日汇总/旬段汇总）
- **结账**：收款登记（金额/日期/方式/备注），欠款 = 累计出货 − 累计收款（按店铺）
- **统计**：工作台卡片、按店（出货/收款/欠款/毛利）、按月/按日、商品出货排行、自定义区间汇总；**出货/进货双口径切换**
- **备份**：自动备份（时间可配 + 保留份数可配）+ **手动一键备份** + **备份历史从存储端直接恢复**（存储端走驱动工厂，可扩展 R2/其他）
- **审计**：登录/添加/删除/导入导出等关键操作留痕（仅老板可查）
...
2. 设置 Actions secrets：
   ...
   - 小程序（可选）：`WX_APPID` + `WX_PRIVATE_KEY`（微信上传；未配置则跳过小程序构建）
3. push main → 自动：创建/复用 D1 → 构建 Flutter Web（注入生产 API 地址）→ `wrangler deploy`；fork 仓库自动降级为「测试 + 小程序/Web」
4. 首次访问 `/` → 创建老板账号
...
- 版本号**手动维护**，四处同步：`frontend-flutter/lib/version.dart`、`src/version.ts`、`frontend-uni/src/version.ts`、`frontend-uni/src/manifest.json`（三段 x.y.z）
- CI 读取 `version.dart` 命名产物并创建 GitHub Release `taozhu-v<版本>`，不自动递增；构建/部署失败自动清理占号 release
- 发版流程：改四处版本文件（含 `tests/api.test.ts` 版本断言）→ push main → Actions 自动构建并发布所有端
- 规则：正式版发布前保持 `0.x` 递增；`1.0.0` 只在正式对外发布时启用
...
.github/workflows/       build-all.yml（8 job：测试/类型检查/Android/Linux(Web)/Windows/macOS(含 iOS)/小程序/部署，产物上传 GitHub Release）
